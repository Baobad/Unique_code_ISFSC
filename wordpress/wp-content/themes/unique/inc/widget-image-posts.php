 ...
 * @since 0.1.0
 */
class Unique_Widget_Image_Posts extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname' => 'unique-image-posts',
			'description' => esc_html__( 'Displays posts from the "image" post format.', 'unique' )
		);
		...
		/* Create the widget. */
		$this->WP_Widget(
			'unique-image-posts',			// $this->id_base
			__( 'Images', 'unique' ),	// $this->name
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title' => esc_attr__( 'Images', 'unique' ),
			'posts_per_page' => 9,
		);
